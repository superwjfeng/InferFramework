...
namespace model {
void LLama2Layers::toCuda(std::shared_ptr<kernel::CudaConfig> config) {
  if (addLayer) {
    addLayer->setCudaConfig(config);
    addLayer->toCuda();
  }

  if (ropeLayer) {
    ropeLayer->setCudaConfig(config);
    ropeLayer->toCuda();
  }

  if (swigluLayer) {
    swigluLayer->setCudaConfig(config);
    swigluLayer->toCuda();
  }

  if (clsLayer) {
    clsLayer->setCudaConfig(config);
    clsLayer->toCuda();
  }

  if (embeddingLayer) {
    embeddingLayer->setCudaConfig(config);
    embeddingLayer->toCuda();
  }

  if (mhaLayer) {
    mhaLayer->setCudaConfig(config);
    mhaLayer->toCuda();
  }

  for (auto& weightLayer : wqLayers) {
    if (weightLayer) {
      weightLayer->setCudaConfig(config);
      weightLayer->toCuda();
    }
  }

  for (auto& weightLayer : wkLayers) {
    if (weightLayer) {
      weightLayer->setCudaConfig(config);
      weightLayer->toCuda();
    }
  ...
  for (auto& rmsnormLayer : rmsnormLayers) {
    if (rmsnormLayer) {
      rmsnormLayer->toCuda();
      rmsnormLayer->setCudaConfig(config);
    }
  }
}

LLama2Model::LLama2Model(base::TokenizerType tokenizerType, std::string tokenPath,
                         std::string modelPath, bool isQuantModel)
    : Model(tokenizerType, base::ModelType::kModelTypeLLaMA2, std::move(tokenPath),
            std::move(modelPath), isQuantModel) {}

base::Status LLama2Model::init(base::DeviceType deviceType) {
  using namespace base;
  if (tokenPath_.empty()) {
    return error::PathNotValid(tokenPath_);
  }
  if (deviceType == base::DeviceType::kDeviceCPU && isQuantModel_) {
    return error::InternalError("The cpu device do not support int8 quant model.");
  }

  deviceType_ = deviceType;
  if (deviceType == DeviceType::kDeviceCUDA) {
    cudaSetDevice(0);
    cudaConfig_ = std::make_shared<kernel::CudaConfig>();
    cudaStreamCreate(&cudaConfig_->stream);
  ...
  Status read_status = genModelFromFile();
  if (!read_status) {
    return read_status;
  }
  initMem();
  if (deviceType_ == base::DeviceType::kDeviceCPU) {
    kernel::sinCosCacheCalcCpu(config_->head_size_, config_->seq_len_,
                                   get_buffer(ModelBufferType::kSinCache).ptr<float>(),
                                   get_buffer(ModelBufferType::kCosCache).ptr<float>());
  } else {
    CHECK_NE(cudaConfig_, nullptr);
    kernel::sin_cos_cache_calc_cu(config_->head_size_, config_->seq_len_,
                                  get_buffer(ModelBufferType::kSinCache),
                                  get_buffer(ModelBufferType::kCosCache), cudaConfig_->stream);
  }

  sampler_ = std::make_unique<sampler::ArgmaxSampler>(deviceType_);
...
base::Status LLama2Model::forward(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
  ...
  if (device_type_ == base::DeviceType::kDeviceCPU && isQuantModel_) {
